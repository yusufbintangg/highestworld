...
import { User, Package, MapPin, Lock, ArrowRight } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
import { supabase } from '../../lib/supabase';

export const AccountDashboardPage = () => {
  const { user } = useAuth();
  const [stats, setStats] = useState({ totalOrders: 0, activeOrders: 0, savedAddresses: 0 });

  useEffect(() => {
    if (!user) return;
    const fetchStats = async () => {
      const [ordersRes, activeRes, addressRes] = await Promise.all([
  ...
  const memberSince = user ? new Date(
    // ambil dari created_at di session kalau ada, fallback ke sekarang
    Date.now()
    ...
    <div className="min-h-screen mt-2 pb-2">
      <div className="max-w-5xl mx-auto px-4">
        ...
        <motion.div
          ...
          className="grid grid-cols-3 gap-8 py-12 border-b border-[var(--border)]"
        >
          <div>
            <p className="text-sm uppercase tracking-[0.15em] text-[var(--text-muted)] h-12 mb-2">Total Orders</p>
            <p className="text-4xl font-light text-[var(--text-primary)] tabular-nums">{stats.totalOrders}</p>
          </div>
          <div>
            <p className="text-sm uppercase tracking-[0.15em] text-[var(--text-muted)] h-12 mb-2">Active</p>
            <p className="text-4xl font-light text-[var(--accent-gold)] tabular-nums">{stats.activeOrders}</p>
          </div>
          <div>
            <p className="text-sm uppercase tracking-[0.15em] text-[var(--text-muted)] h-12 mb-2">Addresses</p>
                  ...
                  <div className="p-8 flex items-center justify-between">
                    <div className="flex items-center gap-6">
                      <div className="p-4 border border-[var(--border)]">
                        <item.icon className="w-6 h-6 text-[var(--text-secondary)]" />
                      </div>
                      <div>
                        <h3 className="text-lg mb-1 group-hover:text-[var(--accent-gold)] transition-colors text-[var(--text-primary)]">
                          {item.title}
                        </h3>
                    ...
                    <div className="flex items-center gap-4">
                      {item.count !== undefined && (
                        <span className="text-2xl font-light tabular-nums text-[var(--text-muted)]">{item.count}</span>
                      )}
